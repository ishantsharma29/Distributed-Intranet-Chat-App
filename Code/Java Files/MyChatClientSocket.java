...
class MyChatCanvasCommand
{
    ...
    MyChatCanvasCommand(short mycommandtype, int mywhodrew, short mycolour,
...
class MyChatClientTickler
    extends Thread
{
    // This class just runs as a separate thread, pinging the client
    // every second to make sure we haven't lost the connection
     
    MyChatClientSocket client;
 
    public MyChatClientTickler(MyChatClientSocket c)
    {
	super("MyChat Chat client connection tickler");
...
public class MyChatClientSocket
    extends Thread
{
    ...
    protected MyChatUser user;
    protected Date connectDate;
    protected MyChatChatRoom chatRoom;
    ...
    public MyChatClientSocket(MyChatServer parent, Socket s, 
			       ThreadGroup threadgroup)
    {
	super("MyChat Chat client connection");

	server = parent;
	mySocket = s;

	// Get a 'user' object
	user = new MyChatUser(server);

	// Make an initial, dummy chat room
	chatRoom = new MyChatChatRoom("", "", false, "");
	...
	try {
		...
		if (command != MyChatCommand.SETPROTO)
		    {
	...
	synchronized (istream) {
	    ...
	    if (protocolVersion >= 2.0)
		{
		    ...
		    // version 2.0.  See the file MyChatCommand.java for
		    ...
		    default:
			{
			    ...
			    // possibly a MyChat Chat 1.x client?  Let's
	...
	// accept connections from a MyChat Chat v1.x client.
	...
	tmpOutput.println("/SERVERMESSAGE This server is incompatible "
			  + "with MyChat Chat 1.x.  Please upgrade your "
    ...
    protected MyChatClientSocket findClientSocket(int userid)
    {
	// This will return the client socket object for the user with the
	// requested ID
	
	MyChatClientSocket tempClient = null;
	MyChatClientSocket returnClient = null;

	for (int count = 0; count < server.currentConnections; count ++)
	    {
		tempClient = (MyChatClientSocket)
	...
	for (int count = 0; count < chatRoom.canvasHistory.size(); count ++)
	    {
		MyChatCanvasCommand command;
		try {
		    command = (MyChatCanvasCommand)
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.CONNECT);
		ostream.writeUTF(userName);
	    }
    }
    
    protected void sendUserInfo(MyChatUser newuser)
	throws IOException
    {
	...
	if (server.requirePasswords)
		...
		try {
			...
			.checkPassword(MyChatServer.userPasswordFileName,
	...
	sendServerText("Welcome to Intranet Chatting version " + MyChat.VERSION +
		       ".\n\n");
	...
	for (int count = 0; count < server.messages.size(); count ++)
	    if (((MyChatMessage) server.messages.elementAt(count))
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.SERVERMESS);
 	...
 	throws IOException
   {
	...
	synchronized (ostream)
		...
		for (int count1 = 0; count1 < server.chatRooms.size();
		     count1 ++)
		    {
			MyChatChatRoom room = (MyChatChatRoom)
 	...
 	throws IOException
   {
	...
	String roomName = "";
	MyChatChatRoom inviteRoom;
	int inviteUserId = 0;
	MyChatClientSocket inviteUser = null;
    ...
    protected void sendEnterRoom(int userId, MyChatChatRoom room)
	throws IOException
    {
	...
	boolean priv = false;
	String password = "";
	MyChatChatRoom newRoom;
	MyChatClientSocket tmpClient = null;
	...
	if (newRoom == null)
		...
		newRoom = new MyChatChatRoom(newRoomName, user.name, priv,
					      password);
		try {
	...
	String roomName = "";
	MyChatChatRoom bootRoom;
	int bootUserId = 0;
	MyChatClientSocket bootUser = null;
	...
	String roomName = "";
	MyChatChatRoom banRoom;
	int banUserId = 0;
	MyChatClientSocket banUser = null;
	...
	String roomName = "";
	MyChatChatRoom allowRoom;
	int allowUserId = 0;
	MyChatClientSocket allowUser = null;
	...
	chatRoom.canvasHistory.addElement(
		  new MyChatCanvasCommand(MyChatCommand.LINE, user.id,
	...
	chatRoom.canvasHistory.addElement(
		  new MyChatCanvasCommand(kind, user.id, colour, x, y, width,
	...
	chatRoom.canvasHistory.addElement(
		  new MyChatCanvasCommand(MyChatCommand.DRAWTEXT, user.id,
					   colour, x, y, (short) 0,
	...
	chatRoom.canvasHistory.addElement(
		  new MyChatCanvasCommand(MyChatCommand.DRAWPICTURE,
					   user.id, (short) 0, x, y,
	...
	for (int count = 0; count < server.currentConnections; count ++)
	    {
		MyChatClientSocket toUser = (MyChatClientSocket)
		    server.connections.elementAt(count);
	...
	server.messages.addElement(new MyChatMessage(whofor, user.name, 
							message));
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.STOREDMESS);
		...
		for (int count = 0; count < server.messages.size(); count ++)
		    {
			message =
			    (MyChatMessage) server.messages.elementAt(count);
