    ...
    extends Thread
{
	...
	synchronized (istream) {
		...
		if (istream.readShort() == MyChatCommand.SETPROTO)
		    receiveProtocol();
		else
		    {
			...
			new MyChatTextDialog(mainWindow, "Warning",
	...
	synchronized (istream) {
	    ...
	    if (protocolVersion >= 2.0)
		{
		    ...
		    case MyChatCommand.NOOP:
			{
			    // Do nothing
			    break;
			}

		    case MyChatCommand.PING:
			{
		    ...
		    case MyChatCommand.RECT:
			{
			    // Someone drew a rectangle on the canvas
			    receivePoly(MyChatCommand.RECT);
			    break;
			}

		    case MyChatCommand.OVAL:
			{
			    // Someone drew an oval on the canvas
			    receivePoly(MyChatCommand.OVAL);
			    break;
			}

		    case MyChatCommand.DRAWTEXT:
			{
		    ...
		    case MyChatCommand.DRAWPICTURE:
			{
		    ...
		    case MyChatCommand.INSTANTMESS:
			{
		    ...
		    default:
			{
			    ...
			    // tried to connect to a MyChat Chat v1.x server
			    // that doesn't speak our language?
			    String stringFoo = new String(foo);
			    if (stringFoo.startsWith("lcome to MyChat"))
				{
				    notifyObsoleteV1();
				    shutdown(false);
	...
	// a MyChat Chat v1.x server, and can't accept a connection from
	// this client.
	new MyChatInfoDialog(parentWindow, "Obsolete server", true,
			      "Cannot communicate with a version 1.x "
			      + "server.  Please ask the server operator to "
			      + "upgrade.");
    }

    protected MyChatUser findUser(int userId)
    {
	// Find a user in the list

	MyChatUser tmpUser = null;
	MyChatUser returnUser = null;
	...
	for (int count = 0; count < userList.size(); count ++)
	    {
		tmpUser = (MyChatUser) userList.elementAt(count);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.SETPROTO);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.PING);
	    }
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.USERINFO);
	...
	if (tmpName.equals(parentWindow.name))
		...
		newUser = new MyChatUser(tmpId, tmpName, "", tmpAdditional);
	...
	new MyChatInfoDialog(parentWindow, "Server message", true,
			      message);
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.DISCONNECT);
	...
	if ((tmpId == parentWindow.id) || (tmpId == 0))
		...
		new MyChatInfoDialog(parentWindow, "Disconnected", true,
				      disconnectMess);
		...
		// If there's a MyChatRoomControl dialog, tell it to update
		// itself
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.ROOMLIST);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.INVITE);
		ostream.writeInt(parentWindow.id);
	...
	new MyChatInfoDialog(parentWindow, "Invitation", false,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.ENTERROOM);
		ostream.writeInt(parentWindow.id);
		ostream.writeUTF(roomName);
	...
	if (userId == parentWindow.id)
		...
		for (int count1 = 0; count1 < userList.size(); count1 ++)
		    {
			tmpUser = (MyChatUser) userList.elementAt(count1);
		...
		if (parentWindow.currentRoom.creatorName
			...
			new MyChatRoomControl(parentWindow);
	...
	// If there's a MyChatRoomControl dialog, get a room list update
	if (parentWindow.roomControlDialog != null)
	    requestRoomList();
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.BOOTUSER);
		ostream.writeInt(parentWindow.id);
		ostream.writeUTF(roomName);
		ostream.writeInt(userId);
	    }
	...
	// they're a goner.
	new MyChatInfoDialog(parentWindow, "Booted", true,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.BANUSER);
		ostream.writeInt(parentWindow.id);
		ostream.writeUTF(roomName);
		ostream.writeInt(userId);
	    }
	...
	// Make a dialog box with a message so that the user notices
	// they're a goner.
	new MyChatInfoDialog(parentWindow, "Banned", true,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.ALLOWUSER);
		ostream.writeInt(parentWindow.id);
		ostream.writeUTF(roomName);
		ostream.writeInt(userId);
	    }
	...
	new MyChatInfoDialog(parentWindow, "Allowed", true,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.ACTIVITY);
		ostream.writeInt(parentWindow.id);
	...
	String tmpString = "";
	if (activity == MyChatCommand.ACTIVITY_DRAWING)
	    tmpString = "drawing: " + fromUser.name;
	else if (activity == MyChatCommand.ACTIVITY_TYPING)
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.CHATTEXT);
		ostream.writeInt(parentWindow.id);
		if (parentWindow.sendToAll.getState())
		    // Public
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.LINE);
		ostream.writeInt(parentWindow.id);
	...
	// Now draw the shape
	if (kind == MyChatCommand.RECT)
	    parentWindow.canvas.drawRect(colour, x, y, width, height, fill,
					 thick, MyChatCanvas.MODE_PAINT);
	else if (kind == MyChatCommand.OVAL)
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.DRAWTEXT);
		ostream.writeInt(parentWindow.id);
		ostream.writeShort(colour);
		ostream.writeShort(x);
		ostream.writeShort(y);
	...
	try {
		...
		{
		    new MyChatInfoDialog(parentWindow, "Error", true,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.DRAWPICTURE);
		ostream.writeInt(parentWindow.id);
		ostream.writeShort(x);
		ostream.writeShort(y);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.CLEARCANV);
		ostream.writeInt(parentWindow.id);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.PAGEUSER);
		ostream.writeInt(parentWindow.id);
	...
	if (parentWindow.menuPlaySound.getState())
		...
		float javaVersion = Float
		    .parseFloat(System.getProperty("java.version")
				.substring(0, 3));
		if (javaVersion < 1.2)
		    sendError(fromUser.id, MyChatCommand.ERROR_NOSOUND);
			...
			try {
			    URL soundURL =
				new URL(parentWindow.MyChatURL.getProtocol(),
					parentWindow.MyChatURL.getHost(),
					parentWindow.MyChatURL.getFile() +
					"MyChatPage.au");
			    ...
			    sendError(fromUser.id,
				      MyChatCommand.ERROR_NOSOUND);
			}
		    }
	    }
	else
	    // The user is not accepting pages.  Send a message.
	    sendError(fromUser.id, MyChatCommand.ERROR_NOPAGE);
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.ERROR);
		ostream.writeInt(parentWindow.id);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.INSTANTMESS);
		ostream.writeInt(parentWindow.id);
	...
	new MyChatTextDialog(parentWindow, "Instant message from "
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.LEAVEMESS);
		ostream.writeInt(parentWindow.id);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(MyChatCommand.READMESS);
		ostream.writeInt(parentWindow.id);
	    }
    }
	...
	if (numberMessages == 0)
	    {
		new MyChatInfoDialog(parentWindow, "None", true,
				      "No unread messages."); 
		return;
	    }
	...
	for (int count = 0; count < numberMessages; count ++)
		...
		// Place it on the screen as a dialog box
		new MyChatTextDialog(parentWindow, "Message from " + from,
			      message, 40, 10,
			      TextArea.SCROLLBARS_VERTICAL_ONLY, true);
	    }

	// Tell the user that that's all
	new MyChatInfoDialog(parentWindow, "Done", true,
	...
	switch(errorCode)
	    {
	    case MyChatCommand.ERROR_NOPAGE:
		{
	    ...
	    case MyChatCommand.ERROR_NOSOUND:
		{
