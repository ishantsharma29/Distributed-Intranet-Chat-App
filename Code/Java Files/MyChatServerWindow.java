...
public class MyChatServerWindow
    extends Frame
    implements ActionListener, ItemListener, WindowListener
{
    ...
    protected MyChatPictureCanvas canvas;
    protected MyChatServer myParent;
    protected MyChatWindow consoleWindow;


    public MyChatServerWindow(MyChatServer parent, String Name)
    {
	super(Name);
	myParent = parent;
	...
	listening = new Label("Port " + myParent.port);
	...
	userAdmin.setBackground(Color.CYAN);
	...
	console = new Button("Administrator");
	...
	disconnectAll.setBackground(Color.CYAN);
	...
	shutdown = new Button("Disconnect Server");
	shutdown.setBackground(Color.CYAN);
	...
	canvas = new MyChatPictureCanvas(this);
	myConstraints.gridwidth = 2; myConstraints.gridheight = 1; 
	...
	try {
	    URL url = new URL ("file", "localhost", "MyChatPic.gif");
	...
	try {
	    URL iconUrl = new URL("file", "localhost", "MyChatIcon.gif");
	...
	if (E.getSource() == userAdmin)
	    {
		MyChatUserToolDialog userTool =
		    new MyChatUserToolDialog(this);
		return;
	    }

	if (E.getSource() == console)
	    {
		MyChatInfoDialog tmp =
		    new MyChatInfoDialog(this, "Loading", false,
			  "Starting the client, one moment please...");
		consoleWindow =
		    new MyChatWindow("Administrator", "", "localhost",
	...
	if (E.getSource() == shutdown)
		...
		if (myParent.currentConnections > 0)
		    new MyChatServerShutdownDialog(this, myParent);
		else
		    myParent.shutdown();
		return;
	    }
	...
	if (E.getSource() == logChat)
		...
		for (int count = 0; count < myParent.chatRooms.size();
		     count ++)
		    {
			MyChatChatRoom tmp = (MyChatChatRoom)
...
class MyChatServerShutdownDialog
    extends Dialog
    implements ActionListener, KeyListener, WindowListener
{
    protected MyChatServerWindow myParent;
    protected MyChatServer theServer;
    ...
    public MyChatServerShutdownDialog(MyChatServerWindow serverwindow,
				       MyChatServer server)
    {
...
class MyChatPictureCanvas
    extends Canvas
{

    private MyChatServerWindow main;
    private Image image;
    
    public MyChatPictureCanvas(MyChatServerWindow mainwindow)
    {
	super();
	main = mainwindow;
	setBackground(Color.lightGray);
