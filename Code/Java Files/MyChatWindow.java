	       ...
	       WindowListener, ClipboardOwner
{
    ...
    protected MyChatRoomInfo currentRoom;
    protected MyChatRoomInfo[] roomInfoArray;
    ...
    protected MyChatPasswordEncryptor passwordEncryptor = null;

    protected MyChatRoomsDialog roomsDialog;
    protected MyChatRoomControl roomControlDialog;

    // socket stuff

    public MyChatClient theClient;
    protected boolean connected;

    // If we're operating as an applet
    protected MyChatApplet thisApplet = null;
    ...
    public MyChatWindow(String userName, String userPassword,
	...
	try {
	    URL imageUrl =
		new URL(MyChatURL.getProtocol(), MyChatURL.getHost(),
		MyChatURL.getFile() + "MyChatPic.gif");
	...
	// applicable
	passwordEncryptor = new MyChatPasswordEncryptor();
	...
	try {
	    URL iconUrl =
		new URL(MyChatURL.getProtocol(), MyChatURL.getHost(),
			MyChatURL.getFile() + "MyChatIcon.gif");
	...
	if (adminConsole)
	    roomOwner(true);
	setTitle("Intranet Chatting v" + MyChat.VERSION + " - online at " + host);
	return;
    }
	...
	currentRoom = new MyChatRoomInfo();
	roomOwner(false);
	setTitle("Intranet Chatting v" + MyChat.VERSION + " - offline");
	return;
    }
	...
	if (!haveRequiredInformation())
		...
		new MyChatSettings(this);
		
		// Do we have it NOW?
		if (!haveRequiredInformation())
		    {
			new MyChatInfoDialog(this, "Connection canceled",
					      true,
	...
	try {
	    theClient = new MyChatClient(host, name, portNumber, this);
	}
	catch (UnknownHostException a) {
	    new MyChatInfoDialog(this, "Couldn't connect", true,
		...
		new MyChatTextDialog(this, "Couldn't connect",
				      errorString, 60, 15,
				      TextArea.SCROLLBARS_VERTICAL_ONLY,
				      true);
		return;
    ...
    protected void setApplet(MyChatApplet ap)
    {
	...
	for (int count1 = 0; count1 < selectedUsers.length; count1 ++)
		...
		for (int count2 = 0; count2 < theClient.userList.size();
		    ...
		    {
			MyChatUser tmp =
			    (MyChatUser) theClient.userList.elementAt(count2);
	...
	try {
	    ...
	    new MyChatInfoDialog(this, "Failed", true,
	...
	if (!pictureFile.canRead())
		...
		new MyChatInfoDialog(this, "Permission denied", true,
	...
	if (E.getSource() == menuMessaging)
	    {
		new MyChatMessagingDialog(this);
		return;
	    }
	...
	if (E.getSource() == menuChatRooms)
		...
		if (roomsDialog != null)
		    roomsDialog.dispose();
		roomsDialog = new MyChatRoomsDialog(this);
		return;
	    }
	...
	if (E.getSource() == menuChatRoomControl)
		...
		if (roomControlDialog != null)
		    roomControlDialog.dispose();
		roomControlDialog = new MyChatRoomControl(this);
		return;
	    }
	...
	if (E.getSource() == menuManual)
	    {
		try {
		    URL manualURL = new URL(MyChatURL.getProtocol(),
				    MyChatURL.getHost(),
				    MyChatURL.getFile() + "MANUAL.TXT");
		    ...
		    new MyChatTextDialog(this, "MyChat Online Manual", 
				  input, 65, 25,
				  TextArea.SCROLLBARS_VERTICAL_ONLY, false);
		    return;
		}
		catch (IOException G) {
		    new MyChatInfoDialog(this, "Data not available", 
			  true,
	...
	if (E.getSource() == menuAbout)
	    {
String abouttext = new String("Intranet Chatting v" + MyChat.VERSION);
		
		new MyChatTextDialog(this, "About Intranet Chatting", 
				      abouttext, 60, 22,
	...
	if (E.getSource() == typed)
		...
		if (E.getKeyCode() == E.VK_ENTER) 
			...
			if (connected == true)
				...
				theClient.sendActivity(MyChatCommand
						       .ACTIVITY_TYPING);
			    }
			    catch (IOException e) {
				theClient.lostConnection();
