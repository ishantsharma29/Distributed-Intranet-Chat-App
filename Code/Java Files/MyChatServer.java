    ...
    extends Thread
{
    ...
    public MyChatServerShutdown(MyChatServer s)
    {
	server = s;
    }
    ...
    extends Thread
{
    ...
    protected URL myURL;
    protected int port;
    protected MyChatServerWindow myWindow;
    ...
    public MyChatChatRoom mainRoom;
    public Vector chatRooms;

    private FileOutputStream log;
    protected MyChatPasswordEncryptor passwordEncryptor;

    protected MyChatClientSocket administratorClient;
    ...
    public MyChatServer(int askport, boolean passwords, boolean newusers,
	...
	if (graphics)
	    {
		myWindow =
		    new MyChatServerWindow(this, "MyChat v"
					    + MyChat.VERSION + " Server");
	...
	mainRoom = new MyChatChatRoom("Main", "Administrator", false, null);
	try {
	...
	if (javaVersion >= 1.3)
	    Runtime.getRuntime()
		.addShutdownHook(new MyChatServerShutdown(this));
    ...
    protected MyChatChatRoom findChatRoom(String roomName)
    {
	// This will return the chat room object with the corresponding
	// name
	
	MyChatChatRoom tempRoom = null;
	MyChatChatRoom returnRoom = null;
    ...
    public boolean isUserAllowed(MyChatChatRoom room,
			       MyChatClientSocket client, String password)
    {
    ...
    public synchronized void disconnect(MyChatClientSocket who,
					boolean notify)
    {
	int count;
	MyChatChatRoom chatRoom;
	...
	for (count = 0; count < currentConnections; count ++)
	    {
		MyChatClientSocket other = (MyChatClientSocket)
	...
	for(count = (currentConnections - 1); count >= 0; count --)
	    {
		MyChatClientSocket temp =
		    (MyChatClientSocket) connections.elementAt(count);
	...
	while (!stop) 
		...
		MyChatClientSocket cs = 
		    new MyChatClientSocket(this, mySocket, myThreadGroup);
	    }
    }
	...
	new MyChatUserTool().createUser(userName, encryptedPassword);
    }
	...
	try {
	    ...
	    while(true)
		{
		    ...
		    messages.addElement(new MyChatMessage(tempFor, tempFrom,
							   tempMessage));
		}
	...
	try {
		...
		{
		    MyChatMessage tempMessage =
			(MyChatMessage) messages.elementAt(count);
	...
	if (currentConnections > 0)
		...
		for (int count = 0; count < currentConnections; count ++)
		    {
			MyChatClientSocket who = (MyChatClientSocket)
			    connections.elementAt(count);
			try {
	...
	// This function can be called by the MyChatServerShutdown thread
	...
	System.out.println("java MyChatServer [" +
	...
	for (int count = 0; count < args.length; count ++)
		...
		if (args[count].equals(portnumberParam))
			...
			System.out.println("\nIntranet Chatting Server: unknown "
					   + "argument " + args[count]);
			System.out.println("Type 'java MyChatServer -help' "
					   + "for usage information");
			System.exit(1);
		    }
	    }
	...
	// Start the server
	server = new MyChatServer(usePort, reqPass, allowNew, useGraphics,
				   logChats);
