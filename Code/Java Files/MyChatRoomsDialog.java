    ...
    implements ActionListener, ItemListener, KeyListener, WindowListener
{
    ...
    MyChatRoomsDialog(Frame parent)
    {
	...
	if (index >= 0)
		...
		if (parentWindow.currentRoom == selectedRoom)
		    {
			new MyChatInfoDialog(parentWindow,
		...
		if (selectedRoom.priv && !selectedRoom.invited)
		    {
			MyChatPasswordDialog passDialog = 
			    new MyChatPasswordDialog(parentWindow,
	...
	if (index >= 0)
		...
		new MyChatTextDialog(parentWindow, "Chat room information",
				      infoString, 30, 10,
	...
	if (E.getSource() == createRoom)
	    {
		new MyChatCreateRoom(parentWindow);
		dispose();
		return;
	    }
