...
public class MyChatUserTool
{
	...
	// MyChatUser objects in the 'users' Vector
	...
	try {
		...
		FileInputStream(MyChatServer.userPasswordFileName));
	    ...
	    while(true)
		{
		    ...
		    // Create the MyChatUser object and append it to
		    // the vector
		    users.addElement(new MyChatUser(0, tempUserName,
						     tempPassword, ""));
		}
	}
	catch (IOException e) {}
	...
	try {
		...
		new DataOutputStream(new
		    FileOutputStream(MyChatServer.userPasswordFileName));

	    for (int count = 0; count < users.size(); count ++)
		{
		    MyChatUser tmpUser =
			(MyChatUser) users.elementAt(count);
	...
	for (int count = 0; count < users.size(); count ++)
	    {
		MyChatUser user = (MyChatUser) users.elementAt(count);
		if (user.name.equals(userName))
		    throw new Exception("User already exists");
	    }

	// Add the user to our Vector
	users.addElement(new MyChatUser(0, userName, encryptedPassword, ""));
	...
	MyChatUser user = null;

	for (int count = 0; count < users.size(); count ++)
	    {
		MyChatUser tmpUser = (MyChatUser) users.elementAt(count);
	...
	for (int count = 0; count < users.size(); count ++)
	    userList[count] = ((MyChatUser) users.elementAt(count)).name;
    ...
    {
	System.out.println("\nMyChat Chat User Tool usage:");
	System.out.println("java MyChatUserTool -create <user name> "
			   + "<password>");
	...
	for (int count = 0; count < args.length; count ++)
		...
		if (args[count].equals("-create"))
			...
			password = new MyChatPasswordEncryptor()
		    ...
		    {
			System.out.println("\nMyChatUserTool: unknown "
					   + "argument " + args[count]);
			System.out.println("Type 'java MyChatUserTool " +
